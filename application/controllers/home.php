...
class Home extends CI_Controller {
    ...
    public function stagioni_precedenti_champions() {

        $this->load->model('mdl_team');
        $this->load->model('mdl_utenti');

        $data['active'] = 5;
        $this->show('home/stagioni_precedenti_champions', $data);
    }
    
    public function stagioni_precedenti_coppa() {
    ...
    public function risultati_champions_precedenti($stagione) {

        $this->load->model('mdl_team');
        $this->load->model('mdl_utenti');

        $data['risultati'] = $this->mdl_team->getCalendarioChampionsPrecedente($stagione);
        $data['stagione'] = $stagione;

        $data['active'] = 5;
        $this->show('home/risultati_champions_precedenti', $data);
    }
    ...
    public function marcatori_precedenti_coppa($stagione) {

        $this->load->model('mdl_team');
        $this->load->model('mdl_utenti');

        $data['bomber'] = $this->mdl_team->getBomberCoppaPrecedenti($stagione);
        $data['stagione'] = $stagione;
        $data['giornata'] = 1;
    ...
    public function dettagli_precedenti_champions($giornata, $stagione) {
        $this->load->model('mdl_team');
        $this->load->model('mdl_utenti');
        $data['giornata'] = $giornata;
        $data['stagione'] = $stagione;
        
        $data['risultati'] = $this->mdl_team->getCalendariogiornataPrecedenteChampions($giornata, $stagione);
